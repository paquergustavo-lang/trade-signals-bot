...
export async function processTechnicalSignal({
  ...
  // Contexto tecnico extra (so vem preenchido pelo indicador Forex Analitico
  // novo - alertas antigos/binarias nao mandam, e tudo fica null nesse caso).
  const technicals = {
            candlePattern: candlePattern || null,
            volumeConfirm: volumeConfirm === true || volumeConfirm === 'true',
            support: toNumberOrNull(support),
            resistance: toNumberOrNull(resistance),
            riskReward: toNumberOrNull(riskReward),
  };
        const hasTechnicals = Boolean(
                  technicals.candlePattern || technicals.support || technicals.resistance || technicals.riskReward
                );
  ...
  const record = {
            ...
            technicals: hasTechnicals ? technicals : null,
            status,
  };
  ...
  const newsLine = `${base} ${baseBias.score >= 0 ? '+' : ''}${baseBias.score} / ${quote} ${quoteBias.score >= 0 ? '+' : ''}${quoteBias.score}`;

  // Quando o sinal vem do indicador Forex Analitico novo (com checklist
  // completo), monta uma mensagem no estilo "analise de trader profissional":
  // tendencia + candle + volume + S/R + risco:retorno + noticias.
  let message;
        if (technicals && marketType === 'forex') {
                  const h4h1 = record.strategy.includes('H4+H1+M15') ? 'H4 e H1 alinhados, cruzamento confirmado no M15' : record.strategy;
                  const srLine =
                              technicals.support != null || technicals.resistance != null
                      ? `Suporte ${technicals.support ?? 'N/A'} / Resistencia ${technicals.resistance ?? 'N/A'}`
                                : 'N/A';
                  message =
                              ...
                              `<b>Analise</b>\n` +
                              `Tendencia: ${h4h1}\n` +
                              `Candle: ${technicals.candlePattern || 'N/A'}\n` +
                              `Volume: ${technicals.volumeConfirm ? 'acima da media (confirma o movimento)' : 'N/A'}\n` +
                              `S/R: ${srLine}\n` +
                              `Noticias: ${newsLine}\n\n` +
                              `<b>Operacao</b>\n` +
                              ...
                              `Risco:Retorno: ${technicals.riskReward != null ? `1:${technicals.riskReward}` : 'N/A'}\n\n` +
